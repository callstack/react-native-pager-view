...
import { requireNativeComponent, Platform, UIManager, Keyboard } from 'react-native';
import ReactNative from 'react-native';
const VIEW_MANAGER_NAME = 'RNCViewPager';
const VIEW_PAGER_REF = 'viewPager';
const ViewpagerViewManager = requireNativeComponent(VIEW_MANAGER_NAME);
...
export class ViewPager extends React.Component {
  constructor(...args) {
    ...
    _defineProperty(this, "setPage", selectedPage => {
      UIManager.dispatchViewManagerCommand(ReactNative.findNodeHandle(this), getViewManagerConfig(VIEW_MANAGER_NAME).Commands.setPage, [selectedPage]);
    });

    _defineProperty(this, "setPageWithoutAnimation", selectedPage => {
      UIManager.dispatchViewManagerCommand(ReactNative.findNodeHandle(this), getViewManagerConfig(VIEW_MANAGER_NAME).Commands.setPageWithoutAnimation, [selectedPage]);
    });

    _defineProperty(this, "setScrollEnabled", scrollEnabled => {
      UIManager.dispatchViewManagerCommand(ReactNative.findNodeHandle(this), getViewManagerConfig(VIEW_MANAGER_NAME).Commands.setScrollEnabled, [scrollEnabled]);
    });
